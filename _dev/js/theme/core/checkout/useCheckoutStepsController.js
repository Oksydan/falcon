...
const useCheckoutStepsController = (stepsSelector = prestashop.selectors.checkout.step) => {
    ...
    const isStepCurrent = (step) => {
        return step.classList.contains(DOMClasses.STEP_CURRENT);
    }

    const isStepUnreachable = (step) => {
        return step.classList.contains(DOMClasses.STEP_UNREACHABLE);
    }

    const hasStepContinueButton = (step) => {
        return step.querySelector('button.continue') !== null;
    }
    ...
    const getCurrentStep = () => {
        return document.querySelector(`${stepsSelector}.${DOMClasses.STEP_CURRENT}`);
    }
